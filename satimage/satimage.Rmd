...
```{r}
# TODO: aggiungere gli install packages
...
svm.tuned.prediction.probs = attr(svm.tuned.prediction, "probabilities")
svm.tuned.result = confusionMatrix(svm.tuned.prediction, testset.y, mode = "prec_recall")
sprintf("Tuned SVM global accuracy is %2.1f", svm.tuned.result$overall["Accuracy"])
sprintf("Tuned SVM metrics by class:")
svm.tuned.result$byClass
```
...
sprintf("Tuned NNET global accuracy is %2.1f", nnet.tuned.result$overall["Accuracy"])
sprintf("Tuned NNET metrics by class:")
nnet.tuned.result$byClass
```

## Confronto modelli

```{r}
# Predice la classe più probabile
...
colnames(testset.multiroc) = c('red_true', 'cotton_true', 'grey_true', 'damp grey_true', 'vegetation_true', 'very damp grey_true', 'red_pred_SVM', 'cotton_pred_SVM', 'grey_pred_SVM', 'damp grey_pred_SVM', 'vegetation_pred_SVM', 'very damp grey_pred_SVM', 'red_pred_NN', 'cotton_pred_NN', 'grey_pred_NN', 'damp grey_pred_NN', 'vegetation_pred_NN', 'very damp grey_pred_NN')
